...
(backend + UI), 4 are partial (built, verification or per-screen work remaining), and 20 are not
started.
...
| # | Feature | Status | Notes |
|---|---------|--------|-------|
| 30 | Localization (i18n): English / Spanish / French | 🟡 | **Designed — [feature-30-localization.md](feature-30-localization.md) (2026-08-07); backend done + client foundation/login built (2026-08-07).** Broadened from "Spanish only" to full localization into **English (default), Spanish, and French**, covering **UI copy and user-facing server errors**, with a **per-user preferred language** (`User.PreferredLanguage`, nullable → English). Two decisions taken up front: **client copy is compile-time `@angular/localize`** — one built bundle per locale served under `/en//es//fr/`, so a language change is a **reload into that build**, not an instant swap (Q-30-1); and **server errors are localized client-side off stable `code`s** (each user-facing `Problem()` carries a machine `code`, the client owns the translated copy, the English `detail` is the fallback) — extends the `code` seed already in the repo (`email_in_use`, `email_not_configured`, `email_unverified`) (Q-30-2). **Built (backend):** `User.PreferredLanguage` + `AddPreferredLanguage` migration, `SupportedLanguages` (pure `en`/`es`/`fr` validator, unit-tested), `ProfileResponse.preferredLanguage`, and `PATCH /api/me/profile` validating it (`400 invalid_language`); **plus the server error-`code` sweep** — an `ErrorCodes` registry (~40 codes) + a shared `CodedProblem` extension in `src/Api/Http/`, with every problem+json error across the 15 controllers and the folder/trash/quota service exceptions now carrying a stable code (uniqueness guarded by a test); plus **locale serving** in `Program.cs` (`/` → `/{locale}/` redirect + per-locale SPA fallbacks). **Built (client):** `@angular/localize` set up (per-locale production build → `browser/{en,es,fr}/`), `LocaleService` + a shared `LanguageSwitcher`, `errorMessage()` (code → localized copy, `detail` fallback), the **whole auth/entry flow localized** (login, forgot/reset password, claim, confirm-email) + the **app shell/chrome** (topbar, search, sidebar, quota, theme) + the **Trash** screen (ICU plurals) + the **Files** feature (grid, menus, modals, move/preview/share dialogs) + the **public share viewer** + the **admin screens** (accounts + email settings) + the **/profile language card** wired to the endpoint, and machine-translated `es`/`fr` catalogs (292 units, `needs-review`, id-complete). **All three phases are built:** every UI screen localized (P1), the credential validators emit field-error **codes** the client localizes (P2), and outbound emails render in the recipient's `PreferredLanguage` via `EmailStrings` (P3, English default/fallback, expiry pluralized in-language). A repo skill **`i18n-translations`** enforces that every UI string lands in all three locales and every server error gets a code + translated message. **Remaining (non-code):** a human pass over the machine-translated `es`/`fr` catalogs + email copy (shipped `needs-review`), a full-stack live run, and the optional signed-in bootstrap auto-redirect |
...
- **Partial (4):** change email (#27) — **backend + Angular UI built and verified live** (the mail-on
  ...
  a real provider key.
- **Localization (#30):** full i18n into English/Spanish/French with a per-user preferred language,
  compile-time `@angular/localize` + client-owned server-error copy
  ([feature-30-localization.md](feature-30-localization.md)); the **backend preferred-language slice
  is built** (`User.PreferredLanguage`, migration, validator, profile endpoint), the client and the
  server error-code sweep remain.
